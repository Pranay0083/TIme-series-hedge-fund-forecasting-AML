#!/usr/bin/env python3
"""
Experiment 15 — PyTorch baseline (BottleneckAutoencoderMLP per horizon)

Hypothesis
    Using the same validation protocol as the LightGBM submission scripts
    (train on ts_index <= VAL_THRESHOLD, report score on holdout) isolates the
    impact of switching from trees to a bottleneck MLP on the deep learning
    feature pipeline (spreads, z-scores, lags, sequential target encoding).

What is held fixed vs tree submissions
    Data paths, horizons, VAL_THRESHOLD, weighted skill metric, and the
    engineered features in 03_feature_engineering.

What this run varies
    Estimator only: HorizonTorchEstimator + weighted MSE with optional early
    stopping on validation skill. Single seed (42).

Outputs (repo root)
    outputs/dl_baseline_torch_results.txt
    outputs/dl_baseline_torch_submission.csv  (when test.parquet exists)

Risks
    One seed; local CV may differ from public leaderboard. Tune in 16/17.
"""
...
def main():
    logger = setup_logging("DLBaselineTorch", "15_dl_baseline_torch.log")

    logger.info("=" * 60)
    logger.info("EXPERIMENT 15: PYTORCH BASELINE (BOTTLENECK MLP PER HORIZON)")
    logger.info("=" * 60)
    ...
    seeds = [42]
    logger.info(f"Torch hyperparameters: {base_torch_kw}")
    logger.info(f"Seeds: {seeds}")

    logger.info("Loading and engineering training data (full sample)...")
    ...
    for hz in HORIZONS:
        ...
        logger.info(f"Rows (train parquet, horizon {hz}): {len(tr_hz):,}")
        ...
        logger.info(f"Fit slice: {len(tr_fit):,} | Val slice: {len(tr_val):,}")
        ...
        for seed in seeds:
            ...
            est = HorizonTorchEstimator(horizon=hz, random_seed=seed, **base_torch_kw)
            est.fit(
                tr_fit,
            ...
            p_val = est.predict(tr_val)
            val_pred += p_val / len(seeds)
            ...
            ckpt = DL_MODELS_DIR / f"15_dl_baseline_torch_horizon_{hz}_seed_{seed}.pt"
        ...
        hz_score = weighted_rmse_score(
            y_hold, val_pred, sample_weight=w_hold
        )
    ...
    for hz, score in horizon_scores.items():
        logger.info(f"  Horizon {hz:2d}: {score:.6f}")

    results_path = OUTPUTS_DIR / "dl_baseline_torch_results.txt"
    with open(results_path, "w") as f:
        f.write("=" * 60 + "\n")
        f.write("EXPERIMENT 15: PYTORCH BASELINE\n")
        ...
        f.write(f"\nVAL_THRESHOLD: ts_index > {VAL_THRESHOLD}\n")
        f.write(f"Seeds: {seeds}\n")
        f.write(f"Torch config: {base_torch_kw}\n")
        f.write(f"Feature count (last horizon processed): {len(feature_cols_last)}\n")

    logger.info(f"Wrote results: {results_path}")

    if test_outputs:
        submission = pd.concat(test_outputs, ignore_index=True)
        sub_path = OUTPUTS_DIR / "dl_baseline_torch_submission.csv"
