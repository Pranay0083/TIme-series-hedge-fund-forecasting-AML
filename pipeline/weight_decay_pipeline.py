#!/usr/bin/env python3
"""
Weight-Decay Per-Horizon Pipeline

Applies exponential temporal decay so older training samples have less
impact on model predictions.

Training sample_weight = competition_weight × decay_factor

Decay formula:
    decay_factor = exp(-λ × (max_ts_index - sample_ts_index))
    λ = ln(2) / half_life

This preserves the competition weight structure (which samples matter)
while making recent data contribute more than older data.
"""
...
# Per-horizon half-lives for temporal decay
# Shorter horizons decay faster (recent data matters more)
DECAY_HALF_LIVES = {
    1: 500,
    3: 700,
    10: 1000,
...
# LOGGING SETUP
# ============================================================================
def setup_logging(log_file: str = "weight_decay_pipeline.log") -> logging.Logger:
    """Configure logging to console and file."""
    logger = logging.getLogger("WeightDecay")
...
def weighted_rmse_score(y_target, y_pred, w) -> float:
    ...
    Uses the competition weight column — NOT decay weights.
    """
...
def compute_decay_weights(
    ...
    """Compute exponential temporal decay weights.
    
    decay_factor_i = exp(-λ × (max_ts - ts_i))
    where λ = ln(2) / half_life
    
    Most recent sample gets weight ≈ 1.0, a sample `half_life` steps
    in the past gets weight ≈ 0.5, and so on.
    
    Args:
        ts_index: time index for each sample
        half_life: number of ts_index steps for weight to halve
    ...
    max_ts = ts.max()
    age = max_ts - ts  # age = 0 for most recent, large for oldest
...
def compute_weighted_ic(feature: np.ndarray, target: np.ndarray, weights: np.ndarray) -> float:
    """Compute weighted Information Coefficient using decay weights."""
...
def analyze_feature_group_ic(
    ...
    """Analyze IC metrics for a feature group using decay weights."""
    ...
    for col in feature_cols:
        ...
        w_ic = compute_weighted_ic(feature, target, decay_weights)
...
# ============================================================================
# PER-HORIZON MODEL TRAINING (with temporal decay weights)
# ============================================================================
def train_horizon_model(
    ...
    Training sample_weight: competition_weight × decay_factor
      -> preserves which samples matter + recent data contributes more
    Eval sample_weight: competition_weight × decay_factor
      -> early stopping aligned with decay-aware metric
    """
    # Effective weights = competition weight × temporal decay
    eff_train = w_train.values * decay_train
    eff_val = w_val.values * decay_val

    logger.info(f"  Effective train weights: min={eff_train.min():.4f}, "
                f"max={eff_train.max():.4f}, mean={eff_train.mean():.4f}")
    logger.info(f"  Effective val weights:   min={eff_val.min():.4f}, "
    ...
    for seed in seeds:
        ...
        # sample_weight = competition_weight × decay_factor
        mdl.fit(
            X_train,
            y_train,
...
# ============================================================================
def main():
    """Main pipeline: per-horizon LightGBM with temporal weight decay."""
    ...
    for hz in HORIZONS:
        ...
        # Define feature columns (exclude weight — not used as feature)
        exclude_cols = {
        ...
        w_fit = tr_df.loc[fit_mask, 'weight']  # competition weight — scoring only

        X_hold = tr_df.loc[val_mask, feature_cols]
        y_hold = tr_df.loc[val_mask, 'y_target']
        w_hold = tr_df.loc[val_mask, 'weight']  # competition weight — scoring only
        ...
        # ======== IC ANALYSIS PER FEATURE GROUP (with decay weights) ========
    ...
    # Final combined score (competition metric)
